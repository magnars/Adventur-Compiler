...
class TimedJumpCoordinatorTestCase < Test::Unit::TestCase
  ...
  def test_should_find_candidates_from_other_rooms_alternatives
    @rooms.add 0, ['(@)199 om 1 ...', '{@}1']
    @rooms.add 1, ['Candidate', '=', 'Alternativ 1', '@1 ? REQ', 'Alternativ 2', '@2']
    assert_candidates_for_executor([199], 1)
    assert_candidates_for_executor([199], 2)
  end
  ...
  def test_should_find_candidates_from_jumps_in_procedure_calls
    @rooms.add 0, ['(@)199 om 1 ...', '(@)1']
    @rooms.add 1, ['Candidate', '@155 ?? BØTTE', '=', 'Alternativ 1', '@1 ? REQ', 'Alternativ 2', '@2']
  ...
  def test_should_use_delay_declarations_from_procedure_calls
    @rooms.add 0, ['(@)1', '=', 'Alternativ 1', '@1 ? REQ', 'Alternativ 2', '@2']
    @rooms.add 1, ['(@)199 om 1 ...']
    assert_candidates_for_executor([199], 1)
    assert_candidates_for_executor([199], 2)
  end

  def test_should_find_candidates_on_deeper_levels
    @rooms.add 0, ['(@)199 om 2 ...', '@1']
    @rooms.add 1, ['@2']
    @rooms.add 2, ['@3']
    assert_candidates_for_executor([199], 1)
    assert_candidates_for_executor([199], 2)
  ...
  def test_should_support_multiple_jumps
    @rooms.add 0, ['(@)844 om 1 ...', '(@)199 om 2 ...', '@1']
    @rooms.add 1, ['@2']
  ...
  def test_should_complain_about_endless_loops
    @rooms.add 199, ['(@)276 om 1 ...', '@1004']
    @rooms.add 276, ['(@)199 om 1 ...', '@1001']
    @rooms.add 1001, ['@1004']
    @rooms.add 1004, ['']
  ...
  def expected_code
    [
      '? $_CALL_DELAY > 0 {',
      '  $_CALL_DELAY--',
      '  (@)199 ? $_CALL_DELAY == 0 og $_DELAYED_CALL == 199',
      '}'
    ]
  end
  ...
  def basic_decall_setup
    ...
    @rooms.add 1, ['(@)5 om 1 ...', '@3']
    @rooms.add 2, ['(@)5 om 2 ...', '=', 'Alt3', '@3', 'Alt4', '@4']
    @rooms.add 3, ['(@)7 om 1 ...']
    @rooms.add 4, dummy_room
    @rooms.add 5, dummy_room
    @rooms.add 7, ['(@)0 om 3 ...', 'Text', '(@)5 om 1 ...', 'More text']
  end

  def dummy_room
    ['Nothing to see here', '=', 'Alt1', '@1', 'Alt2', '@2']
  end
